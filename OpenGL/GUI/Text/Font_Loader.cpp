	...
	if (pFontInstructions.flags & 1) {
	    ...
	    */
		...
		for (size_t w = 0; w < dtSize.x; ++w) {
		    ...
		    size_t pos = (h*size_scale)*preWidth + (w*size_scale);
...
size_t gl::GUI::Text::Initializer::
...
std::pair<size_t, size_t> convertKerning(std::vector<int>& pKerningMap)
{
    ...
    size_t offset = gl::GUI::Text::allKerning.size();
...
gl::GUI::Text::Font integrateFont(LoadFont& pFont) {
    ...
    size_t off = gl::GUI::Text::allFontInstructions.size();
